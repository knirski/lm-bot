...
**Device identity.** `Custom-User-Agent` has the form `Patient Portal; {appVersion}; {deviceUuid}; Android; {apiLevel}; {deviceModel}`. The `deviceUuid` is the account's persisted value (§5.3) rather than a fresh random one per request — stability is hygiene, not a trust requirement, since the spike showed the UUID is not validated at all. `appVersion` is a configuration value; the default is **`5.8.0`**, the oldest value with production evidence at our request shapes (see the 2026-09-20 addendum to the analysis report). Luxmed raises this floor server-side without notice: the July spike measured `4.44.0` as the refresh-grant floor, and in early August 2026 Luxmed also began rejecting `4.42.0` on `NewPortal` reservation calls — even while the password grant still succeeded — so a working login is not evidence that booking works. The reference client's fix was to send `5.8.0`. `AppVersion` rejects configured values below `5.8.0` at startup so a stale default fails loudly instead of surfacing as a runtime 409.
