...
/** Every failure the API can express, with the HTTP status baked in.
  ...
  * endpoint (see AuthEndpoints.errorOut) instead of a `oneOf` variant list that
  * has to be extended in lockstep with this enum.
  */
...
object ApiError:
  /** Rebuild an error from the wire. Unknown codes become `Unexpected` rather
    ...
    * client.
    */
  def fromWire(status: Int, code: String, message: String): ApiError =
    code match
